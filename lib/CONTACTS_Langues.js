...
function Langues(){
	...
	this.members = {	
		...
		///[MEMBER][integer][nId_Langues]Identité de la table
		nId_Langues : 0,
	...
	this.setId_Langues = function(nValue){
		...
		if(typeof nValue === 'number'){
			oLangues.members.nId_Langues = Math.floor(nValue);
			//Happy end
			return true;
		}
	...
	this.loadFromArray = function(ary_){
		//Return the job !
		return legacy(oLangues.members, ary_, true);
	};
	//hack guy !!!
	this.myLangues.loadFromArray = this.loadFromArray;
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Data=" + oLangues.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
