...
function Accreditations(){
	...
	///[METHOD][getId_Accreditations]Method to set the Id_Accreditations
	///[PARAMETER][number][nValue]Our new value for Id_Accreditations
	///[RETURNS]Boolean true if done 
	this.getId_Accreditations = function(nValue){
	...
	///[METHOD][getNiveau]Method to set the Niveau
	///[PARAMETER][number][nValue]Our new value for Niveau
	///[RETURNS]Boolean true if done 
	this.getNiveau = function(nValue){
		//security on null guy !!!
		if(nValue == null)
			return false;
		//security on type guy !!!
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Json=" + oAccreditations.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
