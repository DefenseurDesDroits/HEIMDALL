...
function Noeuds(){
	...
	///[METHOD][getId_Noeuds]Method to set the Id_Noeuds
	///[PARAMETER][number][nValue]Our new value for Id_Noeuds
	///[RETURNS]Boolean true if done 
	this.getId_Noeuds = function(nValue){
		//Return the member
	...
	///[METHOD][getId_Noeuds_Parent]Method to set the Id_Noeuds_Parent
	///[PARAMETER][number][nValue]Our new value for Id_Noeuds_Parent
	///[RETURNS]Boolean true if done 
	this.getId_Noeuds_Parent = function(nValue){
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Json=" + oNoeuds.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
