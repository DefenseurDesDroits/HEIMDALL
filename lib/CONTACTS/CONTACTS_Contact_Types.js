...
function Contact_Types(){
	...
	//hack guy !!!
	this.myContact_Types.setId_Contact_Types = this.setId_Contact_Types;
	...
	//hack guy !!!
	this.myContact_Types.setNom = this.setNom;
	...
	this.loadFromArray = function(ary_){
		//Return the job !
		return legacy(oContact_Types.members, ary_, true);
	};
	//hack guy !!!
	this.myContact_Types.loadFromArray = this.loadFromArray;
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Data=" + oContact_Types.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
