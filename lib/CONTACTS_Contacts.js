...
function Contacts(){
	...
	this.getId_Contacts = function(){
		...
		return oContacts.myNoeuds.getnId_Noeuds();
	};
	//hack guy !!!
	...
	///[METHOD][getPrenom]Method to set the Prenom
	///[PARAMETER][string][sValue]Our new value for Prenom
	///[RETURNS]Boolean true if done 
	this.getPrenom = function(sValue){
	...
	this.getNom = function(sValue){
		...
		if(typeof sValue === 'string'){
			//Never trust the FRONT !!!
			oContacts.members.sNom = sValue.slice(0, 32);
			//Happy end
			return true;
		}
		//Don't fool me next Time !!!
	...
	///[METHOD][getId_Civilites]Method to set the Id_Civilites
	///[PARAMETER][number][nValue]Our new value for Id_Civilites
	///[RETURNS]Boolean true if done 
	this.getId_Civilites = function(nValue){
		//security on null guy !!!
		if(nValue == null)
			return false;
		//security on type guy !!!
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Json=" + oContacts.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
