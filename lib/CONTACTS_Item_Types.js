...
function Item_Types(){
	...
	///[METHOD][getId_item_types]Method to set the Id_item_types
	///[PARAMETER][number][nValue]Our new value for Id_item_types
	///[RETURNS]Boolean true if done 
	this.getId_item_types = function(nValue){
	...
	this.save = function(session, url){
		...
		//set the request header
		oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		oReq.send("Id=0&Session=" + session + "&Action=SAVE&Json=" + oItem_Types.exportToJson()); 
		//Return the job !
		return true;
	};
	//hack guy !!!
