...
function Fichiers(){
	...
	//get the legacy
	legacy(this.members, FichiersmemberSet, true);
	...
	this.getId_Fichiers = function(){
		...
		return oFichiers.myNoeuds.getId_Noeuds();
	};
	//hack guy !!!
	...
	this.getFile = function(){
		return this.members.oFile;
	}
	//hack guy !!!
	this.myFichiers.getFile = this.getFile;
	...
	this.setId_Fichiers = function(nValue){
		//Return the member
		return oFichiers.myNoeuds.setnId_Noeuds(nValue);
	};
	//hack guy !!!
	...
	this.setFile = function(oFileItem){
		this.members.oFile = oFileItem;
	}
	//hack guy !!!
	this.myFichiers.setFile = this.setFile;
	...
	this.save = function(session, url){
		...
		oReq.onreadystatechange = function(){
			...
			if(oReq.readyState == 4 && oReq.status == 200){
				//our file
				var oFileItem = oFichiers.getFile();
				if(oFileItem != null){
					//load the value and the Id
					oFichiers.loadFromJson(oReq.responseText);
					//reset the file !
					oFichiers.setFile(oFileItem);
					//set the file cause i'm parano
					oFichiers.saveFile(session, url);
				}
				else
					oFichiers.loadFromJson(oReq.responseText);
			}
	...
	this.saveFile = function(session, url){
		//Our request object
		var oReq = new XMLHttpRequest();
		//our formdata
		...
		//add the action !
		oFd.append("Action", "publications_fichiers");
		//add the argPart
		oFd.append("Args", JSON.stringify({id : oFichiers.getId_Fichiers(), user_id : session}));
		//add the File 
		oFd.append("Fichier", oFichiers.getFile());
		//add the File 
		oFd.append("Id", oFichiers.getId_Fichiers());

		//check the open
		oReq.open("POST", url + "/php/Download_Fichiers.php", true);
		//set the request header
		//oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
		//Just send the stuff
		oReq.send(oFd);
