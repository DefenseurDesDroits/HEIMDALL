...
function CivilitessaveFromJson($jsonObj, $oAgent){
	//Our object declaration
	$oCivilites = new Civilites();
...
function CivilitesgetAllInstance(){
	...
	while($nLine < $nCount){
		...
		$oCivilites = new Civilites();
		//load the data
		$oCivilites->loadFromArray($ary_[$nLine], true);
		//add the data
		$ary_Result[$nLine] = $oCivilites->exportToArray();
		//Next
		$nLine++;
	}
...
function CivilitesManager(){
	...
	switch($sAction){
		case "GET" :
			...
			break;
		case "SAVE" :
			echo CivilitessaveFromJson($sJson, $oAgent);
			break;
		case "DELETE" :
