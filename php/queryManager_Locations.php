...
Const HEIMDALL_QM_LOCATIONS_Debug = false;
//Const HEIMDALL_QM_LOCATIONS_Debug = true;
...
function searchQuery($Args){
    ...
    if(HEIMDALL_QM_LOCATIONS_Debug)
        file_put_contents(dirname(__FILE__) . "/../logs/queryManager_Locations@searchQuery.log", "DEBUG !!!\r\n" );

    //coreespondance set 
    $ary_Corres = $oLocation->getCorrespondanceArray();

    //recreate the query
    $sQuery = "SELECT DISTINCT " . $oLocation->getColumns() . "\r\n" ;
    $sQuery .= "FROM " . $oLocation->getTable() . "\r\n"  ;

    //add the link between column s and foreign Key
    //$sQuery .= "WHERE Id_Locations <> 0 ";
    $sQuery .= "WHERE " . $oLocation->getLinkConditions(true);
    ...
    $sQuery .= "\r\n" . "ORDER BY xxx.Locations.Nom";

    //get the array (so ugly way bro !!!)
    $GLOBALS["oConnection"]->open();
    $ary_ = $GLOBALS["oConnection"]->selectRequest($sQuery, explode(", ", ($oLocation->getColumns() . "") ), null);
    ...
    while($nLine < $nCount){
        ...
        //do it
        $oLocation = new Locations();
        $oLocation->loadFromArray($ary_[$nLine], true);
        $ary_Obj[$nLine] = $oLocation->exportToArray();
    ...
    //debugging, the desperate way
    if(HEIMDALL_QM_LOCATIONS_Debug)
        file_put_contents(dirname(__FILE__) . "/../logs/queryManager_Locations@searchQuery.log", "The Query : \r\n " . $sQuery . "\r\n",  FILE_APPEND );
