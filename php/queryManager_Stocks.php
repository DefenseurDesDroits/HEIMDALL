...
Const HEIMDALL_QM_STOCKS_Debug = false;
//Const HEIMDALL_QM_STOCKS_Debug = true;
...
function searchQuery($Args){
    ...
    if(HEIMDALL_QM_STOCKS_Debug)
        file_put_contents(dirname(__FILE__) . "/../logs/queryManager_Stocks@searchQuery.log", "DEBUG !!!\r\n" );

    //coreespondance set 
    $ary_Corres = $oStock->getCorrespondanceArray();

    //recreate the query
    $sQuery = "SELECT DISTINCT " . $oStock->getColumns() . "\r\n" ;
    $sQuery .= "FROM " . $oStock->getTable() . "\r\n"  ;

    //add the link between column s and foreign Key
    //$sQuery .= "WHERE Id_Stocks <> 0 ";
    $sQuery .= "WHERE " . $oStock->getLinkConditions(true);
    ...
    //$sQuery .= "\r\n" . "ORDER BY xxx.Stocks.Nom";

    //get the array (so ugly way bro !!!)
    $GLOBALS["oConnection"]->open();
    $ary_ = $GLOBALS["oConnection"]->selectRequest($sQuery, explode(", ", ($oStock->getColumns() . "") ), null);
    ...
    while($nLine < $nCount){
        ...
        //do it
        $oStock = new Stocks();
        $oStock->loadFromArray($ary_[$nLine], true);
        $ary_Obj[$nLine] = $oStock->exportToArray();
    ...
    //debugging, the desperate way
    if(HEIMDALL_QM_STOCKS_Debug)
        file_put_contents(dirname(__FILE__) . "/../logs/queryManager_Stocks@searchQuery.log", "The Query : \r\n " . $sQuery . "\r\n",  FILE_APPEND );
