...
///[FUNCTION][contact_groupsDoQueryComplete]Function to manage the research for every case
...
function contact_groupsDoQueryComplete(sIdWaiting, ptrFunctionCreateArg, ptrFunctionResponse){
...
///[FUNCTION][contact_groupsCreateArgs]Function to plot line of contacts
///[RETURNS][Array of Arguments]Array of arguments
function contact_groupsCreateArgs(){
...
///[FUNCTION][contact_groupsCreateSelectionField]Function to plot line of contacts
///[PARAMETER][String][sNext]Next field name
///[RETURNS][Boolean]True if done
function contact_groupsCreateSelectionField(sNext){
    //our code
    var sCode = "";
    ...
    sCode += "\t" + "</select>" + "\r\n";
    sCode += "\t" + '<input id="SAI_Value_' + sNext + '" class="SAI_" type="text" name="SAI_search_Query" value="" onkeyup="contact_groups_KeySearch(event)"/>';
...
///[FUNCTION][contact_groupsAddSelectionField]Function to plot line of contacts
///[RETURNS][Boolean]True if done
function contact_groupsAddSelectionField(){
    ...
    //Add it
    oElement.innerHTML += contact_groupsCreateSelectionField(sNext);
...
///[PARAMETER][Integer][nLength]The numbers of ploted groups
///[RETURNS][Boolean]True if done
function contact_groupsPlots(nStart, nLength){
    //our element
    var oElement = null;
    ...
    //get the count
    nCount = Heimdall.members.products.contacts.Groups.length;
    //
    ...
    if(oElement != null){
        ...
        while(nLine < nCount){
            //get the user
            oGroups = Heimdall.members.products.contacts.Groups[nLine];
            //add the code
            sCode += "<div id=\"Groups_Line_" + oGroups.getId_Groups() + "\" onclick=\"contact_groupsClick(" + nLine + ")\" class=\"OPT_Groups\">" + oGroups.getNomGroupe() + "</div>";
            //Next
            nLine++;
        }
        //BTN_ for the previous
        if(nStart >= nLength){
            nStart -= HEIMDALL_NUMBER_OF_GROUPS_MAX;
            sCode += "<div id=\"BTN_Previous_Groups\" onclick=\"contact_groupsPlots(" + nStart + ", " + nLength + ")\">Afficher les " + nLength + " précédents</div>";
        }
        //BTN_ for the next ten
        if(nCount < Heimdall.members.products.contacts.Groups.length)
            sCode += "<div id=\"BTN_Next_Groups\" onclick=\"contact_groupsPlots(" + nCount + ", " + nLength + ")\">Afficher les " + nLength + " suivants</div>";
...
///[FUNCTION][contact_groupsClick]Function to manage the click on the user menu
///[PARAMETER][Integer][nLine]The line of the user 
function contact_groupsClick(nLine){
    //our groups layout
    var LAY_ = new LAY_Groups();
    //our contact
    var oContacts = null;
    //our element
    var oElement = null;
    ...
    if(nLine >= 0){
        //in the range ?
        if(nLine < Heimdall.members.products.contacts.Groups.length){
            //get the PNL_
            oElement = document.getElementById("PNL_Win");
            //clean the element
            oElement.innerHTML = "";
            //get the contact
            oContacts = Heimdall.members.products.contacts.Groups[nLine];
            //init
            LAY_.init("PNL_Win", "Groups" +  oContacts.getId_Groups(), oContacts);
            //plots
            LAY_.ObjToView();
        }
    }
...
///[FUNCTION][contact_groupsDoQueryResponse]Function to manage the research response from the server
///[PARAMETER][String][sText]our response text to parse as Json
function contact_groupsDoQueryResponse(sText){
    //the groups
    var oGroups = null;
    ...
    //Plots !
    return contact_groupsPlots(0, HEIMDALL_NUMBER_OF_GROUPS_MAX);
}

///[FUNCTION][contact_groupsDoQuery]Function to exceute the research
function contact_groupsDoQuery(){
    //our element
    var oElement = null;
    //call the generique function
    contact_groupsDoQueryComplete("LAY_List_User", contact_groupsCreateArgs, contact_groupsDoQueryResponse);
    //get the PNL_
    oElement = document.getElementById("PNL_Win");
    //clear the layout
    if(oElement != null)
...
///[FUNCTION][contactMenu_groups_CLICK]Function to generate the HTML of the sub-menu
function contactMenu_groups_HTML(){
    //our code
    var sCode = "";
    ...
    sCode += '\t<div id="LAY_Title" class="LAY_ heim_Block"><h1>Groupes</h1></div>\r\n';
    sCode += '<br/>\r\n';
    sCode += '\t<div id="LAY_Query" class="LAY_ heim_Right heim_Block">';

    sCode += '\t\t<input id="SAI_search_Query" class="SAI_" type="text" name="SAI_search_Query" value="" onkeyup="contact_groups_KeySearch(event)"/>';
    sCode += '\t\t<button id="BTN_Search_Query" class="BTN_" type="button" onclick="contact_groupsDoQuery()">Q</button>';
    ...
    sCode += '\t<button id="BTN_Add_Query_Condition" class="BTN_" type="button" onclick="contact_groupsAddSelectionField()">+</button>';
...
///[FUNCTION][contactMenu_groups_CLICK]Function to manage the click on the sub-menu
function contactMenu_groups_CLICK(){
    //get the content layout
    var oElement = document.getElementById("LAY_Content");

    oElement.innerHTML = contactMenu_groups_HTML();

    Heimdall_Contacts.menuClassUpdate("OPT_Contacts_Groups");
